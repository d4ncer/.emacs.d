;;; rk-flycheck-stylelint.el --- Integrate stylelint with flycheck
...
(defun rk-web--project-or-root-stylelintrc ()
  "Return either project .stylelintrc or root."
  (let ((root (projectile-project-p))
        (base-rc (f-join user-emacs-directory "lisp" ".stylelintrc.json")))
    (if root
        (let* ((root-rc (f-join root ".stylelintrc"))
               (root-rc-json (f-join root ".stylelintrc.json")))
          (cond ((f-file? root-rc) root-rc)
                ((f-file? root-rc-json) root-rc-json)
                (t base-rc))))
    base-rc))

(flycheck-def-config-file-var flycheck-stylelintrc
    (css-stylelint) (rk-web--project-or-root-stylelintrc)
  :safe #'stringp)
...
(provide 'rk-flycheck-stylelint)

;;; rk-flycheck-stylelint.el ends here
