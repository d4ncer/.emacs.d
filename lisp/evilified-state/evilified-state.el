...
(defun evilified-state--setup-normal-state-keymap ()
  ...
  (define-key evil-normal-state-map [escape] 'evilified-state))
...
(add-hook 'evilified-state-entry-hook 'evilified-state--evilified-state-on-entry)

(defvar evilified-state-map
...
(defvar evilified-state-map-original (copy-keymap evilified-state-map))
...
One or several EXPRESSIONS with the form `KEY FUNCTION':
   KEY1 FUNCTION1
   KEY2 FUNCTION2
These bindings are set directly in evilified-state-map submap.
   ...
         ...
         (body
          (progn
            ...
            `(
              ...
              (setq evilified-state-map (copy-keymap ,evilified-map))
...
(defun evilified-state--configure-default-state (mode)
  "Configure default state for the given MODE."
